...
export default class Enemy extends Phaser.GameObjects.Sprite {
  ...
  activated() {
    ...
    if (!this.beenSeen) {
      ...
      if (
        this.x <
        this.scene.cameras.main.scrollX + this.scene.sys.game.canvas.width + 32
      ) {
        this.beenSeen = true;
  ...
  verticalHit(enemy, dude) {
    // quick check if a collision between the enemy and dude is from above.
    if (!dude.alive) {
      return false;
    }
    return (
      dude.body.velocity.y >= 0 &&
      dude.body.y + dude.body.height - enemy.body.y < 10
    );
  }

  hurtdude(enemy, dude) {
    // send the enemy to dude hurt method (if dude got a star this will not end well for the enemy)
    this.scene.dude.hurtBy(enemy);
  }
